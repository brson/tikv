...
mod test {
    ...
    use crate::storage::lock_manager::{DetectorScheduler, DetectTask, DetectType};
    ...
    use crate::storage::{Command, Options, Mutation};
    ...
    #[derive(Clone)]
    struct MockMsgScheduler(Arc<Mutex<Option<Box<dyn FnMut(Msg) + Send + 'static>>>>);
    ...
    fn make_snap(fx: &Fixture) -> BTreeEngineSnapshot {
        ...
        let snap_cb = Box::new(move |(_ctxt, new_snap): (CbContext, KvResult<BTreeEngineSnapshot>)| {
    ...
    fn must_ex_cmd(fx: &Fixture, cmd: Command) {
        ...
        let e: Executor<BTreeEngine, _> = Executor::new(mock_sched, fx.pool.clone(),
    ...
    fn must_prewrite_put_pessimistic(fx: &Fixture, kvs: &[(Key, Vec<u8>)]) {
        ...
        let cmd = Command::Prewrite {
            ctx: kv_ctxt,
            mutations: kvs.to_owned().into_iter().map(|kv| Mutation::Put(kv)).collect(),
    ...
    fn next_dead_task(dead_iter: &mut stream::Wait<fmpsc::UnboundedReceiver<Option<DetectTask>>>) -> DetectTask {
        let dead_task = dead_iter.next().expect("detector msg");
        let dead_task = dead_task.expect("detector task ok").expect("detector task some");
        dead_task
    }
    ...
    fn notify_deadlock_detector_on_pessimistic_commit() {
        ...
        let keys = vec![Key::from_raw("k".as_bytes())];
        let kvs: Vec<_> = keys.clone().into_iter().map(|k| (k, "v".as_bytes().to_owned())).collect();
